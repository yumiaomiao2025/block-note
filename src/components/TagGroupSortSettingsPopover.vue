  ...
  <Teleport to="body">
    ...
    >
      ...
      <div class="p-3 border-b border-gray-100 bg-gray-50">
        <h3 class="font-semibold text-gray-800 text-sm">{{ t('tagManager.tagGroupSortSettings') }}</h3>
      </div>
      ...
      <div class="p-4 space-y-4">
        <div>
          <label class="block text-xs text-gray-600 mb-2">
            {{ t('tagManager.tagGroupSort') }}
          </label>
          <select 
            v-model="store.tagGroupSortOrder"
          ...
          >
            <option value="manual">{{ t('tagManager.sortCustom') }}</option>
            <option value="alphabetical">{{ t('tagManager.sortName') }}</option>
            <option value="usage">{{ t('tagManager.sortUsage') }}</option>
          </select>
        </div>
        
        <div>
          <label class="block text-xs text-gray-600 mb-2">
            {{ t('tagManager.tagInGroupSort') }}
          </label>
          <select 
            v-model="store.tagInGroupSortOrder"
            class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
          >
            <option value="manual">{{ t('tagManager.sortCustom') }}</option>
            <option value="alphabetical">{{ t('tagManager.sortName') }}</option>
